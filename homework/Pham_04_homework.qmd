...
# YOUR CODE HERE
library(lme4)
model <- glmer(chosen_voicing ~ correct_voicing + (1 + correct_voicing| listener) + (1 | item_pair) + (1 | voice), data = incomplete, family = binomial)
summary(model)
...
# YOUR CODE HERE
model_null <- glmer(chosen_voicing ~ 1 + (1 | listener) + (1 | item_pair) + (1 | voice), 
                    data = incomplete, family = binomial)

anova(model_null, model, test = "Chisq")
# I have used GLMM to analyse the perception of sound. The dependent variable is chosen_voicing and is predicted by the fixed effect, correct_voicing. To control the non-independence of observations, the model included a maximal random effect structure, including random intercepts for listener, item_pair, and voice. Using random effects helps the model separate the variance of individuals and stimuli from the main effect.

# From the results of the model, it shows that correct_voicing has a high statistical value. The Estimate is 0.52, which is a positive value predicting that when correct_voicing changed from voiced to voiceless, there was a higher chance participants chose voiceless. While the intercept is -0.03 (p = 0.908), this means that there isn't any response bias toward one side during the experiment; the participants reacted based on the sound stimuli rather than blindly choosing an answer.

# Based on the random effects, the voice variance is 0.79, which shows that the quality of pronunciation or the phonetic details of the voice affected significantly whether participants recognized the sound correctly. To re-evaluate the validity of the theory, I have conducted a likelihood ratio test by comparing the full model with a null model only consisting of random effects. The result shows that the model with correct_voicing explains the data better and more clearly. The significant decrease of AIC from 7130 to 7052 once again confirms that correct_voicing is a necessary factor to explain how listeners process sound in this experiment.
```
...
library(ggeffects)
library(ggplot2)
pred_data <- predict_response(model, terms = "correct_voicing")
ggplot(pred_data, aes(x=x,y= predicted)) +
  geom_point(size = 4, color = "royalblue") + geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.1, color = "royalblue") +
  labs(
    title = "Predicted Probability of Choosing 'Voiceless'",
    subtitle = "Based on Intended Voicing (Correct Voicing)",
    x = "Intended Voicing (Speaker's Intent)",
